...
namespace qi {
  ...
  void EventLoopAsio::start(int nthread)
  {
    ...
    if (nthread == 0)
    {
      ...
      if (nthread < 3)
        nthread = 3;
      std::string envNthread = qi::os::getenv("QI_EVENTLOOP_THREAD_COUNT");
      if (envNthread.size() > 0)
        nthread = strtol(envNthread.c_str(), 0, 0);
    }
  ...
  void EventLoopAsio::_runPool()
  {
    ...
    while (true) {
      try
      {
        ...
        break;
      } catch(const detail::TerminateThread&) {
        break;
      ...
      boost::function<void ()> cb)
  {
    ...
    if (delay > Duration::zero())
    {
	  ...
	  boost::asio::steady_timer::duration d = boost::chrono::duration_cast<boost::asio::steady_timer::duration>( delay );
